/**
 * VirtualizedSpeakerTimeline Component
 *
 * A virtualized version of SpeakerTimeline that uses react-window
 * to efficiently render only visible speaker tracks and segments.
 * This maintains constant performance regardless of the number of
 * speakers or speaking segments in long meetings.
 *
 * Key features:
 * - Uses FixedSizeList for speaker tracks (each track is same height)
 * - Only renders segments within the visible time range
 * - Maintains all existing features: zoom, pan, seek, playhead
 * - Falls back to regular rendering for small track counts (<10)
 */

import { useMemo, useRef, useState, useCallback, useEffect, CSSProperties, ReactElement } from 'react'
...
/** Minimum number of tracks before virtualization is enabled */
const TRACK_VIRTUALIZATION_THRESHOLD = 10

/** Height of each speaker track row */
...
export interface VirtualizedSpeakerTimelineProps {
...
interface TrackRowData {
  ...
  currentTimeMs: number
  onSegmentClick: (e: React.MouseEvent, startMs: number) => void
}

// ============================================================================
// Virtualized Track Row Component (for react-window v2)
// ============================================================================

function VirtualizedTrackRow({
  index,
  style,
  tracks,
  visibleRange,
  ...
  index: number
  style: CSSProperties
} & TrackRowData): ReactElement | null {
        ...
        {track.segments.map((segment, idx) => {
          ...
          return (
            <div
              ...
              onClick={(e) => onSegmentClick(e, segment.startMs)}
...
export function VirtualizedSpeakerTimeline({
  transcripts,
  speakers,
  ...
  maxHeight = 300,
  className
}: VirtualizedSpeakerTimelineProps) {
  ...
  const useVirtualization = tracks.length >= TRACK_VIRTUALIZATION_THRESHOLD
  ...
  }), [tracks, visibleRange, currentTimeMs, handleSegmentClick])

  // Calculate the actual height needed
  const actualHeight = useMemo(() => {
    const neededHeight = tracks.length * TRACK_HEIGHT
    return Math.min(neededHeight, maxHeight)
  }, [tracks.length, maxHeight])
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm mt-0.5 text-blue-700 dark:text-blue-300">
              Only one speaker was identified in this recording.
            </p>
          </div>
        </div>
      )}
        ...
        >
          {useVirtualization ? (
            <List<TrackRowData>
              ...
              rowComponent={VirtualizedTrackRow as any}
              rowProps={trackRowData}
              overscanCount={OVERSCAN_COUNT}
              className="space-y-1.5"
              style={{ height: actualHeight, overflowX: 'hidden' }}
            />
          ) : (
            // Regular rendering for small track counts
...
export default VirtualizedSpeakerTimeline
