...
export function NewMeetingModal({ isOpen, onClose, onSuccess }: NewMeetingModalProps) {
  ...
  const validateForm = (): boolean => {
    ...
    // Scheduled time is no longer required per requirements
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const input = {
        ...
        start_time: formData.scheduledTime ? new Date(formData.scheduledTime).toISOString() : new Date().toISOString(),
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="New Meeting" size="md">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label htmlFor="scheduledTime" className="block text-sm font-medium text-foreground mb-1.5">
            Scheduled Time <span className="text-xs text-muted-foreground">(optional)</span>
          </label>
          <input
            type="datetime-local"
            id="scheduledTime"
